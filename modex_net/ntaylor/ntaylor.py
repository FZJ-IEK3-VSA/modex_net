...
Edited: Chloi Syranidou (IEK-3, Jülich Research Center)
        Add fontsize and linewidth arguments as well as legend
"""
...
def diagn(ax, statn, prop, labels=None, sigma_lim=None, sigma_color='k',
  ...
  """
    ...
    if not sigma_lim:
        sigma_lim = max([1.25, max(statn[:, 4])]) * 1.05
    ...
    for rho in np.arange(.25, sigma_lim + .125, .25):
        ...
        y = rho * np.sin(theta)
        if rho == 1.:
            ax.plot(x, y, color=sigma_color, linewidth=.8 * linewidth)
        else:
            ax.plot(x, y, '--', color=sigma_color, linewidth=.5 * linewidth)
    ax.set_yticks(np.arange(0., sigma_lim + .125, .25))
    ax.spines['left'].set_color(sigma_color)
    ax.set_yticklabels([round(i, 2) for i in ax.get_yticks()], fontsize=fontsize)
    if no_sigma_axis:
        ...
        ax.set_ylabel('normalized standard deviation', fontsize=fontsize)

    # centered root mean square error
    for rho in np.arange(0.25, sigma_lim + .125, 0.25):
        # full circles
        ...
        ax.plot(x, y, linestyle=(0, (5, 10)), color=crmse_color, linewidth=.5 * linewidth)
    ...
    if no_crmse_axis:
        ...
        ax.set_xlabel('normalized centered root mean square error', fontsize=fontsize)
    ...
    for i in range(len(x)):
        ...
        ax.plot(x[i], y[i],
                ...
                markersize=fontsize,
                markeredgewidth=fontsize / 4,
                label=labels[i])

    if labels:
        ax.legend(fontsize=fontsize, ncol=ncol, bbox_to_anchor=(1., 1.))
